...
exports.create = (req, res, next) => {
    // Validate request
    Student.create({ firstName: req.body.firstName.trim(), lastName: req.body.lastName.trim(), phoneNumber: req.body.phoneNumber.trim(), email: req.body.email.trim(), password: req.body.password }).catch(function (err) {
        var array = err.message.split(',');
        /*const emailExist = Student.findOne({ where: { email: req.body.email.trim() } });
        if (!emailExist) {
